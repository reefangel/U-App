...
function CheckExpansion()
{
	...
	if ((EM & 1<<0) != 0)
	{
		for (a=0;a<6;a++)
		{
			$("#dimmingtab").append("<label for='pwme"+a+"input' id='pwme"+a+"n'></label>");
			$("#dimmingtab").append("<div class='dimminglabel' id='pwme"+a+"'>0%</div>");
...
function refresh_slider()
{
	if(localStorage.getItem("pwmdo")!=null)
	{
		...
		{
			$('#pwmdn').html(localStorage.getItem("pwmdn")+" - Overriden");
		...
		{
			$('#pwmdn').html(localStorage.getItem("pwmdn"));
			if ($('#pwmdinput').hasClass("ui-mini"))
			{
		...
		{
			$('#pwman').html(localStorage.getItem("pwman")+" - Overriden");
		...
		{
			$('#pwman').html(localStorage.getItem("pwman"));
			if ($('#pwmainput').hasClass("ui-mini"))
			{
